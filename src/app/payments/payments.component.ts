...
export class PaymentsComponent implements OnInit {
  ...
  selectOptions: selectOptions[] = [{ id: 'Card', name: 'Card' }];
  ...
  fetchBalance() {
    this.paymentService.getBalance().subscribe((x) => this.balance.set(x));
  }

  fetchLoanStats() {
    this.paymentService
      .getLoanStats()
      .subscribe((val: any) => this.loanStats.set(val));
  }
      ...
      if (x) {
        control.patchValue(
          this.currencyPipe.transform(
            control.value.replace(/\D/g, '').replace(/^0+/, ''),
            'USD',
            'symbol',
            '1.0-0'
          ),
  ...
  createLoanForm() {
    this.loanAmount = new FormControl('', [
      Validators.required,
      this.maxLoanValueValidator.bind(this),
    ]);
  ...
  handlePayment() {
    ...
    this.toggleAddBeneficiary() &&
      this.paymentService.addBeneficiaries(beneficiaryPayload).subscribe();

    if (this.paymentChannel.value === 'Card') {
      this.paymentService
        .createPaymentSession(payload.user, payload.amount)
        .subscribe((x: any) => window.open(x.session.url, 'blank'));
    } else {
  ...
  validateAccount(control: AbstractControl): Promise<any> | Observable<any> {
    if (!(String(control.value).length > 6))
      return of({ InvalidAccountNumber: true });
    return this.paymentService.accountLookup(control.value).pipe(
      map((val: any) => {
  ...
  handleLoanRequest() {
    ...
    this.paymentService.requestLoan(this.loanDetails).subscribe(
      (x: any) => {
  ...
  handleFundRequest() {
    ...
    this.paymentService
      .createFundingSession(this.fundDetails.amount)
      .subscribe((x: any) => window.open(x.session.url, 'blank'));
