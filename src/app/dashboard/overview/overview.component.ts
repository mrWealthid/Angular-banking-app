...
export class OverviewComponent implements OnInit {
  ...
  private checkTransactions: IDashboardData[];
  ...
  ngOnInit() {
    ...
    this.dashboardService
      .getLoanStats()
      .subscribe((val) => this.loanStats.set(val));
    this.dashboardService
      .getDailyHighlight()
      .subscribe((val) => this.dailyStats.set(val));
    this.dashboardService
      .getMonthlyHighlight()
      .subscribe((val) => this.monthlyStats.set(val));

    this.currentUser$.subscribe((x) => {
      if (x?.role === 'admin')
        this.dashboardService
          .getUserStats()
          .subscribe((val) => this.userStats.set(val));
    });
  }
  ...
  constructor() {
    effect((onCleanup) => {
      const mytimer = setTimeout(() => {
        //This conditional checks for transactions blueprint for a year
        if (
          this.checkTransactions[0].data.length < 1 &&
          this.checkTransactions[1].data.length < 1
        )
          this.toggleModal();
      }, 5000);
  ...
  fetchMonthlyUserStats() {
    this.series = forkJoin([
      this.dashboardService.getMonthlyUserStatsData('user'),
      this.dashboardService.getMonthlyUserStatsData('admin'),
    ]).pipe(
      map((data) => {
  ...
  fetchMonthlyLoanStats() {
    this.series = forkJoin([
      this.dashboardService.getMonthlyLoanStatsData('PENDING'),
      this.dashboardService.getMonthlyLoanStatsData('APPROVED'),
      this.dashboardService.getMonthlyLoanStatsData('DECLINED'),
    ]).pipe(
      map((data) => {
        return data.map((x) => x[0]);
      })
  ...
  fetchMonthlyTransactionChannelStats() {
    this.series = forkJoin([
      this.dashboardService.getMonthlyTransactionChannelStatsData('Transfer'),
      this.dashboardService.getMonthlyTransactionChannelStatsData('Card'),
    ]).pipe(
      map((data) => {
        return data.map((x) => x[0]);
      })
  ...
  fetchMonthlyTransactionsStats() {
    this.series = forkJoin([
      this.dashboardService.getMonthlyStatsData('Credit'),
      this.dashboardService.getMonthlyStatsData('Debit'),
    ]).pipe(
      map((data) => {
        const res = data.map((x) => x[0]);
        this.checkTransactions = res;
        return res;
      })
    );
  }
  ...
  fetchStatsData(val?: IStatsParam) {
    this.dashboardService
      .getStatsData(val?.type, val?.time)
      .subscribe((x: ISummary) => (this.summary = x));
  }
